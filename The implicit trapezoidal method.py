...
def myfun_y(t,y):
    return 5*math.exp(5*t)*(y-t)**2+1

def myfun(t):
    return t - math.exp(-5*t)
def fun(x,w,t):
    return w + h/2*myfun_y(t,w) + h/2*(5*math.exp(5*(t+h))*(x-(t+h))**2 + 1) - x 
def my_deri(x,t):
    return h/2*5*math.exp(5*(t+h))*2*(x-(t+h))-1
